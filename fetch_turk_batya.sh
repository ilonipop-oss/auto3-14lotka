...
grep -E "vless://[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}" temp_vless.txt | head -12 > "yusuf_hotdog_reality.txt"

# Статистика
echo "$(date): 🦃 Загружено $(wc -l < "yusuf_hotdog_reality.txt") турецких VLESS" >> "yusuf_hotdog_reality.txt"
rm -f temp_vless.txt

echo "✅ Турецкие батек готовы: $(wc -l < "yusuf_hotdog_reality.txt") шт."
